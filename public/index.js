...
db.collection('requests').orderBy('date').onSnapshot(snapshot => {
  ...
  changes.forEach(change => {
    if (change.type == 'added') {
      ...
      let found = list.find( doc => doc.id === change.doc.id);
      if ( found ) {
...
function updateReceived() {
  if (filters.length > 0) {
    filteredList = list.filter(doc => filters.includes(doc.data().status));
  } else {
    filteredList = [...list];
  }
  requestList.innerHTML = '';
  filteredList.forEach(doc => renderRequest(doc));
}
...
document.querySelector('#legende').addEventListener('click', (e) => {
